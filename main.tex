...
% Margens de display math (formulas separadas do texto: $$, \\[, align*, etc):
% Convencao tipografica: belowdisplayskip > abovedisplayskip (olho desce, precisa mais respiro).
\setlength{\abovedisplayskip}{12pt plus 3pt minus 3pt}
\setlength{\belowdisplayskip}{18pt plus 4pt minus 4pt}
\setlength{\abovedisplayshortskip}{6pt plus 3pt minus 3pt}
\setlength{\belowdisplayshortskip}{12pt plus 4pt minus 4pt}
...
% QR code para pagina de errata participativa (back matter)
\usepackage{qrcode}

% Indice remissivo (\index{termo} no body + \printindex no back matter)
\usepackage{makeidx}
\makeindex
...
% Implementacao sem tikz overlay (mais deterministica),
% aspas curvas italicas em 72pt, cinza 30%.
\definecolor{aspaclara}{cmyk}{0,0,0,0.30}        % aspa em ~30% de tinta
...
\newcommand{\pullquote}[2][]{%
  ...
  \begin{minipage}{0.78\linewidth}%
    % Aspa esquerda, alinhada a esquerda, italica grande
    {\noindent\color{aspaclara}\fontsize{72pt}{0pt}\selectfont\itshape%
     \textquotedblleft}%
    \par\vspace{-1.2em}%
    % Texto centralizado, italico, corpo maior
    \begin{center}
      \large\itshape #2
    \end{center}
    \par\vspace{-1.4em}%
    % Aspa direita, alinhada a direita, italica grande
    {\hfill\color{aspaclara}\fontsize{72pt}{0pt}\selectfont\itshape%
     \textquotedblright\par}%
    % Atribuicao opcional, smallcaps cinza medio
    \ifx&#1&\else
      \par\vspace{0.8em}%
      \begin{center}
        {\footnotesize\scshape\color{atribuicaocinza} #1}%
      \end{center}
    \fi
  \end{minipage}%
  \end{center}%
...
\newcommand{\secgfe}[1]{%
    \needspace{10\baselineskip}%  ← garante espaco antes de iniciar (vspace + filete + titulo + conteudo)
    \vspace{2.5em}%
...
% --- Front matter: paginas em algarismos romanos minusculos ---
\frontmatter
\maketitle

\include{cipfichacatalografica}

\tableofcontents

\include{00dedicatoria}
\include{prefacio}
\include{00comousar}
\include{convencoes}
\include{constantes}

% --- Main matter: capitulos numerados, paginas em arabico desde 1 ---
\mainmatter
\include{00introducao}
...
\include{intropart1}
\include{partIcinematica01escalar}
\include{partIcinematica02vetor}
...
\include{intropart2}
\include{partIIdinamica01newton}
\include{partIIdinamica02energia}
...
\include{intropart3}
\include{partIV0gravidade}
\include{partIVhidrostatica}
...
\include{apendice_eqsexpvsdem}
\include{apendiceareagraficovxt}
\include{apendiceteoremaenergiacinetica}
\include{apendicearte}

% --- Back matter: bibliografia, errata, indice ---
\backmatter
